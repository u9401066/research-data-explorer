...
![11-Phase 工作流程細部圖](docs/figures/03-workflow-detail.svg)
...
如果你想讓 agent 更自主地驅動 EDA，現在可以在 Phase 3 和 Phase 4 中間先跑 `propose_analysis_plan()`。它不再只是單純 greedy 排序，而是會先做 draft candidate selection，再做內部 methodology review / repair；當 review 判斷有值得保留的延伸 EDA 路線時，也可以超過初始 budget 做 soft expansion。最後它會把 schema、變數角色、研究問題壓成一份經過自我檢查的候選分析排序、visualization bundle，以及一份可供 Phase 6 直接參考的 execution schedule，再輸出成可直接送進 `register_analysis_plan()` 的 blueprint。它的目的不是繞過用戶確認，而是把「該先做哪些分析」這件事變成可審查、可縮減、可鎖定的 artifact。

另外，`register_analysis_plan(confirm=true)` 現在也會在鎖定前做方法學審查。如果 plan 明顯低於資料結構所需的最低覆蓋，例如只有兩三個分析就想結束，但資料明明有 group、相關結構與可做 multivariable model 的條件，Phase 4 會先自動補入 optional exploratory branches；只有補完之後仍然太薄時，才會擋下來，要求先回到 `propose_analysis_plan()` 補足，除非你明確用 `allow_methodology_override=true` 覆蓋。計畫一旦鎖定，也會同步保存 execution schedule artifact，讓 Phase 6 可以沿著 reviewed blueprint 的順序跑，而不是每一步都重新即興決定。
...
如果你不做這兩步，後面 Phase 6 的完整受約束分析就不成立；如果中途改方法，也應該用 `log_deviation` 留下理由。
...
- 使用者在 Phase 6 主要會碰到 6 個分析入口加 1 個視覺化入口：`compare_groups`、`analyze_variable`、`correlation_matrix`、`generate_table_one`、`run_advanced_analysis`、`run_repeated_measures`、`create_visualization`
- Phase 4 的 analysis plan 目前允許登記 16 種 analysis types
- 本地 Scipy engine 在 manifest 中宣告 16 類能力
- automl-stat-mcp 委派層在 manifest 中宣告 10 類進階能力
...
- Phase 3 和 Phase 4 要先把研究問題翻成可治理 contract，Phase 6 才照計畫執行
