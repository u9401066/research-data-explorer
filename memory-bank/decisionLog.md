...
| 2026-07-15 | Preserve analyst-authored report sections across Phase 10/12 regeneration | Project-specific multiplicity and clinical interpretation must not be discarded by study-specific heading allowlists; a SHA-256 manifest records preserved sections. |
| 2026-07-15 | Describe MCAR conservatively as not rejected, never proven by the indicator heuristic | Absence of detected numeric associations does not exclude group/time-dependent availability or MNAR; the locked missing strategy and denominator sensitivity remain authoritative. |
| 2026-07-15 | Exclude Markdown alignment rows before report/DOCX table rendering | Separator syntax is presentation metadata, not an analytic observation; exporting it as data breaks table fidelity. |
| 2026-07-01 | Prefer statsmodels logistic inference for real-world model sizes | The `>200 rows or >8 columns` fast-path silently dropped p-values and CIs for essentially every real research model; raising the thresholds to 50000 rows / 15 columns restores auditable inference while keeping the numpy fast path only for genuinely large or high-cardinality fits. |
| 2026-07-01 | Prioritize readiness-closing provenance branches for budget survival | The derived-variable provenance branch was generated but ordered late, so a budget-truncated autoresearch run dropped it and report readiness could never reach production-ready; stable-sorting provenance branches to the front closes the Readiness->Queue loop without weakening any governed gate. |
