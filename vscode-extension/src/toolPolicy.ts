...
export const FULL_REPORT_CHAT_QUERY =
    '@rde /fullreport 請使用完整 11-Phase auditable workflow，從專案建立、資料收件、raw structure gate、schema、concept alignment、plan lock、readiness、分析、collect_results、assemble_report、run_audit 到 auto_improve，產出完整分析報告。';
...
export function buildPipelineExecutionPrompt(userPrompt: string, edaSkill: string | null): string {
    const preamble = [
        ...
        '2. Follow the 11-Phase Pipeline: Setup → Intake → Schema → Concept → Plan → Pre-check → Execute → Collect → Report → Audit → Improve.',
        ...
        '6. If Phase 3 or Phase 4 needs confirmation, stop and ask through the tool result rather than guessing.',
        '7. If the request cannot be completed with an RDE MCP tool, report the blocker instead of answering from general knowledge.',
        '8. Never write code, edit files, or inspect the workspace as a substitute for using the RDE MCP workflow.',
        '',
