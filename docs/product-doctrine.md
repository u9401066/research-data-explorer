...
RDE 的角色是 MCP + harness plugin，不是取代研究者的自主科學家服務。
Agent 可以自由提出假設、比較研究設計、質疑分析方法，並以 `agent_proposals`
把清單以外的研究問題帶入探索。執行契約協助重現與保留證據，不限制思想。

臨床可信度需同時呈現個案集合、配對方式、效果量、不確定性、缺失與偏誤限制。
成功呼叫工具不代表研究有效；p 值小不代表品質高；未顯著結果也可以是完整證據。
探索分支、失敗分析與只有記錄但未執行的想法必須分開。Audit 是流程檢查，
不是臨床有效性或發表資格認證。
